var ok = require('../js/geojson_remove.js');
...
exports.gunner = function(query, id, remove) {
  ...
  db.all(query[0], query[1], function(err, row) {
    ...
    if (row != null) {
      ...
      if (row.length == 1) {
        ...
        ok.computers(arreglo, id, removePostrado, removeRecordatorio);
      }
      ...
      if (row.length > 1) {
        ...
        ok.computers(arreglo, id, remove);
      }
